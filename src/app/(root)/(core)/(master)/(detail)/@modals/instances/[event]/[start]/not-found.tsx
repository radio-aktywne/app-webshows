...
import { ModalsInstancesEventStartNotFoundView } from "./not-found.view";

async function getTitle({}: NotFoundMetadataUtilityInput = {}) {
  return msg({ message: "Instance not found • tulip" });
}
...
export default async function ModalsInstancesEventStartNotFound({}: NotFoundInput) {
  await connection();

  return (
    <>
      <Metadata title={await getTitle()} />
      <ModalsInstancesEventStartNotFoundView />
    </>
  );
}
